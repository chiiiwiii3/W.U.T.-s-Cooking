...
/* W.U.T.'s Cooking requested starter upgrades */
(function(){
 const KEY='wuts_cooking_extras_v2', defaults={ate:[],favorites:[],addedRecipes:[],allergies:[],intake:[],shoppingWindow:'day'};
 let s={...defaults}; try{s={...defaults,...JSON.parse(localStorage.getItem(KEY)||'{}')}}catch(e){};
 const save=()=>localStorage.setItem(KEY,JSON.stringify(s));
 const esc=x=>String(x||'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
 const diets=["I'm not sure","Omnivore","Vegetarian","Vegan","Pescatarian","Flexitarian","Keto","Low-carb","Mediterranean","Paleo","Whole-foods","Gluten-free","Dairy-free","Low-FODMAP","Halal","Kosher","Carnivore","Plant-forward","High-protein","Anti-inflammatory"];
 const blood=["I'm not sure","O+","O-","A+","A-","B+","B-","AB+","AB-"];
 function setupSelects(){document.querySelectorAll('select').forEach(x=>{let t=(x.name+' '+x.id+' '+x.className).toLowerCase();if(t.includes('diet')){let v=x.value;x.innerHTML=diets.map(a=>`<option value="${esc(a)}">${esc(a)}</option>`).join('');if(diets.includes(v))x.value=v}if(t.includes('blood')){let v=x.value;x.innerHTML=blood.map(a=>`<option value="${esc(a)}">${esc(a)}</option>`).join('');if(blood.includes(v))x.value=v}})}
 function optionalNutrition(){document.querySelectorAll('input').forEach(x=>{let t=(x.placeholder+' '+x.name+' '+x.id).toLowerCase();if(/calorie|protein|carb|fat|fiber|vitamin|mineral/.test(t)){x.required=false;if(!x.dataset.wutOpt){x.dataset.wutOpt=1;let n=document.createElement('div');n.className='wut-optional-note';n.textContent="Optional — leave blank if you don't know your calories, protein, carbs, vitamins, or minerals. W.U.T.'s Cooking can use your profile and goals.";x.parentElement&&x.parentElement.appendChild(n)}}})}
 function allergyInput(){let els=[...document.querySelectorAll('input,textarea')].filter(x=>(x.placeholder+' '+x.name+' '+x.id).toLowerCase().includes('allerg'));els.forEach(x=>{if(x.dataset.wutFree)return;x.dataset.wutFree=1;let n=document.createElement('input');n.className='wut-allergy-input';n.placeholder='Add an allergy not listed (example: sesame, shellfish, kiwi)';n.value=s.allergies.join(', ');n.onchange=()=>{s.allergies=n.value.split(',').map(a=>a.trim()).filter(Boolean);save()};x.parentElement&&x.parentElement.appendChild(n)})}
 function nav(){document.querySelectorAll('form').forEach(f=>{if(f.dataset.wutNav)return;f.dataset.wutNav=1;let row=document.createElement('div');row.className='wut-screen-actions';let b=document.createElement('button');b.type='button';b.className='wut-back';b.textContent='← Back';b.onclick=()=>history.back();let k=document.createElement('button');k.type='button';k.className='wut-skip';k.textContent='Skip';k.onclick=()=>{f.querySelectorAll('input,select,textarea').forEach(x=>x.removeAttribute('required'));f.requestSubmit?f.requestSubmit():f.dispatchEvent(new Event('submit',{bubbles:true,cancelable:true}))};row.append(b,k);f.appendChild(row)})}
 function cardKey(c){let h=c.querySelector('h1,h2,h3,h4,.title,.meal-title');return (h?h.textContent:c.textContent).trim().replace(/\s+/g,' ').slice(0,120)}
 function mealButtons(){document.querySelectorAll('article,.meal-card,.recipe-card,.card').forEach(c=>{if(c.dataset.wutMeal)return;let txt=c.textContent.toLowerCase();if(!/breakfast|lunch|dinner|meal|recipe/.test(txt))return;let key=cardKey(c);if(!key)return;c.dataset.wutMeal=1;let r=document.createElement('div');r.className='wut-meal-actions';let a=document.createElement('button');a.className='wut-ate';a.textContent=s.ate.includes(key)?'✓ Ate it':'Ate it';a.onclick=()=>{if(s.ate.includes(key)){s.ate=s.ate.filter(x=>x!==key)}else{s.ate.push(key);s.intake.unshift({meal:key,date:new Date().toISOString()})}save();a.textContent=s.ate.includes(key)?'✓ Ate it':'Ate it';renderHistory()};let f=document.createElement('button');f.className='wut-fav';f.textContent=s.favorites.includes(key)?'★ Favorited':'☆ Add to favorites';f.onclick=()=>{s.favorites.includes(key)?s.favorites=s.favorites.filter(x=>x!==key):s.favorites.push(key);save();f.textContent=s.favorites.includes(key)?'★ Favorited':'☆ Add to favorites'};r.append(a,f);c.appendChild(r)})}
 function renderHistory(){let b=document.querySelector('#wut-intake-history');if(!b){b=document.createElement('section');b.id='wut-intake-history';b.className='wut-history';(document.querySelector('main')||document.body).appendChild(b)}b.innerHTML='<h3>Previous Intake</h3>'+(s.intake.length?s.intake.slice(0,10).map(x=>`<div>${esc(x.meal)} <small>— ${new Date(x.date).toLocaleDateString()}</small></div>`).join(''):'<div class="wut-optional-note">Meals marked “Ate it” appear here.</div>')}
 function shopping(){let h=[...document.querySelectorAll('h1,h2,h3')].find(x=>/shopping/i.test(x.textContent));if(!h||document.querySelector('#wut-shop-controls'))return;let r=document.createElement('div');r.id='wut-shop-controls';r.className='wut-shop-controls';[['day','Day'],['week','Week'],['2week','2 Weeks']].forEach(([v,t])=>{let b=document.createElement('button');b.textContent=t;if(v===s.shoppingWindow)b.classList.add('active');b.onclick=()=>{s.shoppingWindow=v;save();r.querySelectorAll('button').forEach(x=>x.classList.remove('active'));b.classList.add('active');let note=document.querySelector('#wut-shop-note');if(note)note.textContent='Ingredient quantities shown for '+t+'.'};r.appendChild(b)});let note=document.createElement('div');note.id='wut-shop-note';note.className='wut-optional-note';note.textContent='Choose Day, Week, or 2 Weeks for the ingredient list. Each item should show a quantity and specific type/variety when available.';h.parentElement&&h.parentElement.appendChild(r);r.after(note);document.querySelectorAll('li,div,p,span').forEach(x=>{if(x.children.length)return;if(x.textContent.trim().toLowerCase()==='plant protein')x.textContent='Plant protein — choose tofu, tempeh, lentils, chickpeas, beans, or a labeled plant-protein product'})}
 function recipeFilter(){let h=[...document.querySelectorAll('h1,h2,h3')].find(x=>/recipes/i.test(x.textContent));if(!h)return;document.querySelectorAll('article,.recipe-card,.card').forEach(c=>{let k=cardKey(c);if(k&&!s.favorites.includes(k)&&!s.addedRecipes.includes(k))c.style.display='none'})}
 function run(){setupSelects();optionalNutrition();allergyInput();nav();mealButtons();renderHistory();shopping();recipeFilter()}
 new MutationObserver(run).observe(document.documentElement,{childList:true,subtree:true});document.addEventListener('DOMContentLoaded',run);setTimeout(run,500);setTimeout(run,1500);
})();
